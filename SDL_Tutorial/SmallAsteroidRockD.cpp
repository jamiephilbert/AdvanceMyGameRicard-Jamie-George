...
void SmallAsteroidRockD::CheckScreenBounds()
{
	if (Position().x < -11) {
...
SmallAsteroidRockD::SmallAsteroidRockD() :Texture("SmallAsteroidRock4.png", 0, 0, 27, 30)
{
...
SmallAsteroidRockD::~SmallAsteroidRockD()
{
	m_pRandom = nullptr;
}

void SmallAsteroidRockD::Update()
{
	Translate(Vector2(randVelocityX, randVelocityY), GameEntity::World);
	CheckScreenBounds();
}

void SmallAsteroidRockD::Render()
{
	Texture::Render();
}
