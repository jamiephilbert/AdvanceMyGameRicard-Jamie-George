...
void SmallAsteroidRockC::CheckScreenBounds()
{
	if (Position().x < -11) {
...
SmallAsteroidRockC::SmallAsteroidRockC() :Texture("SmallAsteroidRock3.png", 0, 0, 26, 26)
{
...
SmallAsteroidRockC::~SmallAsteroidRockC()
{
	m_pRandom = nullptr;
}

void SmallAsteroidRockC::Update()
{
	Translate(Vector2(randVelocityX, randVelocityY), GameEntity::World);
	CheckScreenBounds();
}

void SmallAsteroidRockC::Render()
{
	Texture::Render();
}
